 ...
 *
 * @Block(
 *   id = "jellomatrix_spliced_matrix_harmonics_block",
 *   admin_label = @Translation("Jellomatrix Spliced Matrix Harmonics Block"),
 *   category = @Translation("JelloMatrix block")
 * )
 */
class JellomatrixSplicedMatrixHarmonicsBlock extends BlockBase {
   ...
   */
  public function defaultConfiguration($increment_original, $harmonics, $tone, $interval, $frequency, $print) {
  ...
  // First output the original harmonics with $columns and $rows.
    ...
    $intro .= '<div class="endtable begintext"><h2>ODD/EVEN: Differences and Harmonics</h2>';
    $intro .= '<p>These increment calculations show the relationships of the numbers in the grid by relating them to the ones in front of them (forward) and behind them (backwards) using the "tone" value as the base in the numbering system.</p><div class="endtext"><br></div>';
    $intro .= '<p>The diagonal increments still go down the row, but show the relationships between the number and the one diagonally above (forward) it and below it (backward).</p><div class="endtext"><br></div>';
    $intro .= '<p>The bold letters at the end of each row represent the Lambdona Notes that the ratios the repeating increments create.</p><div class="endtext"><br></div>';

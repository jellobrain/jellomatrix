...
//use Drupal\jellomatrix\Services\Query\JellomatrixPrimeOffestMatrix;
//use Drupal\jellomatrix\Services\Query\JellomatrixDoubleflipResponseOffsetMatrix;
//use Drupal\jellomatrix\Services\Query\JellomatrixDoubleflipSplicedOffsetMatrix;
...
class JelloMatrixABHADoubleFlipOffsetResultForm extends FormBase {
  ...
  /**
   * @var \Drupal\jellomatrix\Services\Query\JellomatrixPrimeOffsetMatrix
   */
  protected $prime_matrix;
  
  /**
   * @var \Drupal\jellomatrix\Services\Query\JellomatrixDoubleflipResponseOffsetMatrix
   */
  protected $doubleflip_response_offset_matrix;
  
  /**
   * @var \Drupal\jellomatrix\Services\Query\JellomatrixDoubleflipSplicedOffsetMatrix
   */
  protected $doubleflip_spliced_offset_matrix;
   ...
   */
  public function __construct(AccountInterface $account, $get_colors, $harmonics, $increments_derivative, $increments_prime_derivative, $increments_original, $prime_offset_matrix, $doubleflip_response_offset_matrix, $doubleflip_spliced_offset_matrix, $wave_detection, $wave_preparation, $sound_files, $circle_grids, $primes, $grid_prime_matrix, $grid_spliced_matrix, $grid_spliced_derivative_even_odd, $grid_spliced_derivative_harmonics, $grid_spliced_derivative_primes, $grid_spliced_derivatives, $grid_spliced_even_odd, $grid_spliced_harmonics, $grid_spliced_primes, $grid_spliced_scale_patterns, $grid_spliced_wave_forms) {
    ...
    $this->doubleflip_response_offset_matrix = $doubleflip_response_offset_matrix;
    $this->doubleflip_spliced_offset_matrix = $doubleflip_spliced_offset_matrix;
  ...
  public static function create(ContainerInterface $container) {
    ...
    $prime_offset_matrix = $container->get('jellomatrix.jellomatrix_prime_offset_matrix');
    $doubleflip_response_offset_matrix = $container->get('jellomatrix.jellomatrix_doubleflip_response_offset_matrix');
    $doubleflip_spliced_offset_matrix = $container->get('jellomatrix.jellomatrix_doubleflip_spliced_offset_matrix');
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $tone = NULL, $interval = NULL, $offset = 0, $frequency = NULL, $print = NULL) {
    ...
    if (in_array('administrator', $roles)) {
      $print_options = ['none', 'singles', 'pairings', 'complete', 'rifeharmonics'];
      $form['print'] = array(
    ...
    $prime_matrix = $this->prime_offset_matrix->getPrimeOffsetMatrix($tone, $interval, $offset);
    $response_matrix = $this->doubleflip_response_offset_matrix->getABHADoubleflipResponseOffsetMatrix($tone, $interval, $offset);
    $spliced_matrix = $this->doubleflip_spliced_offset_matrix->getABHADoubleflipSplicedOffsetMatrix($response_matrix, $prime_matrix, $tone, $interval, $offset);

    
    extract($this->wave_preparation->getWavePreparation($response_matrix, $tone, $interval, $spliced_matrix));
    ...
    $increments_prime = $this->increments_prime_derivative->getIncrementsPrimeDerivative($response_matrix, $tone);

...
class JellomatrixDoubleflipSplicedOffsetMatrix {
  ...
  public function getABHADoubleflipSplicedOffsetMatrix($response_matrix, $prime_matrix, $tone, $interval, $offset) {
    $spliced_bt = array();
    ...
    for ($i = 1; $i <= $intoff; $i++) {
      $count = 1;
      for ($t = 1; $t <= $tone; $t++) {
        if ($response_matrix[$i][$t]['count'] == $offset + 1) {
          $response_matrix[$i][$t]['spliced_count'] = $response_matrix[$i][$t]['count']*$response_matrix[$i][$t]['column'];
          $spliced_bt[$i][$count] = $response_matrix[$i][$t];
          ...
          if (isset($response_splice_record)) {
            if (isset($prime_splice_record)) {
              $response_matrix[$i][$t]['spliced_count'] =  $prime_splice_record + $interval;
